    ...
    C: Send + Sync + 'static,
{
    ...
    loop {
        let (mut subscription_mqtt_client, event_loop) = MqttClient::new(&configuration.mqtt);
        mqtt_client_subscribe(subscription_list, &mut subscription_mqtt_client).await;
